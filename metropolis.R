...
sand_line <- function(a, b, n) {
  df <- data.frame(alpha = runif(n, 0, 1)) %>%
...
interpolate_line <- function(a, b, theta, delta) {
  ...
  df <- list(df1, df2)
  tf <- tween_states(df, tweenlength = 2, statelength = 0, ease = "linear", nframes = 20)
  tf
}
...
for(i in 1:nrow(edges)) {
  ...
  delta <- sqrt(sum((b - a)^2)) * runif(1, 0.25, 10)
  temp <- interpolate_line(a, b, theta, delta) %>% select(-`.frame`)
  perp <- rbind(perp, temp)
}

for(i in 1:nrow(perp)) {
  temp2 <- sand_line(c(perp$x[i], perp$y[i]), c(perp$xend[i], perp$yend[i]), 5)
  sand <- rbind(sand, temp2)
}
...
make_instance <- function(a, b) {
  ...
  interpolate_line <- function(a, b, theta, delta) {
    ...
    df2 <- data.frame(x = a[1] + dx, y = a[2] + dy, xend = b[1] + dx, yend = b[2] + dy)
    df <- list(df1, df2)
    tf <- tween_states(df, tweenlength = 2, statelength = 0, ease = "linear", nframes = 200)
    tf
  }
  ...
  # b <- runif(2)
  theta <- atan((b[2] - a[2]) / (b[1] - a[1])) + pi/2
  delta <- sqrt(sum((b - a)^2)) * runif(1, 0.25, 1)
  ...
  for(i in 1:nrow(test)) {
    ...
    points <- points %>% rbind(sandpaint_line(a, b, 50000 / i) %>% mutate(id = i))
  }
  points
}
...
p <- ggplot() +
  ...
  geom_point(aes(x, y, alpha = alpha), sand, size = 1.5, colour = "white") +
  xlim(0, 10000) +
  ylim(0, 10000) +
  coord_equal() +
  scale_size_continuous(range = c(0.5, 2)) +
  scale_color_viridis() +
  theme_blankcanvas(margin_cm = 0, bg_col = "gray15")
